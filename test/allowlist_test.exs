defmodule Despamilator.AllowlistTest do
  ...
  setup do
    on_exit(fn -> Application.delete_env(:despamilator, :url_allowlist) end)
    :ok
  end

  describe "host_allowed?/1" do
    test "exact match" do
      Application.put_env(:despamilator, :url_allowlist, ["pakwheels.com"])
    ...
    test "regex entry" do
      Application.put_env(:despamilator, :url_allowlist, [~r/\.gov$/])
  ...
  describe "other URL filters respect allowlist" do
    test "IPAddressURL skipped for allowlisted ip" do
      Application.put_env(:despamilator, :url_allowlist, [~r/^12\.34\.56\.78$/])
      assert_filter_score(IPAddressURL, "go to http://12.34.56.78/", 0)
    end

    test "VeryLongDomainName skipped" do
      Application.put_env(:despamilator, :url_allowlist, ["cje6cgsllk-0ds3nnto7dj.com"])
    ...
    test "SpammyTLDs skipped" do
      Application.put_env(:despamilator, :url_allowlist, ["blahdee.biz"])
